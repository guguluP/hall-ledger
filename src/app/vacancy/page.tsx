...
const MOCK_ROOMS = [
  ...
  },
  {
    ...
    freeFrom: "10:30",
    freeUntil: "13:30",
    nextBooking: "13:30 – Lab",
  },
];

const fieldClass =
  "t-input h-11 w-full rounded-xl border border-border bg-elevated px-3.5 text-[15px] text-fg outline-none transition-shadow focus:ring-2 focus:ring-[var(--color-accent)] focus:ring-offset-1 focus:ring-offset-[var(--color-bg)]";
...
export default function VacancyPage() {
  ...
  const handleSearch = async () => {
    ...
    if (btnLabelRef.current) {
      await swapText(btnLabelRef.current, "Searching…");
    }
    ...
    setLoading(false);
    if (btnLabelRef.current) {
      await swapText(btnLabelRef.current, "Search");
    }
  ...
  return (
    <AppShell
      title="Find a room"
      subtitle="Uses the published timetable across all halls"
    >
      <section className="mb-6 overflow-hidden rounded-2xl border border-border bg-surface">
        <div className="border-b border-border px-5 py-4">
          ...
          <p className="mt-0.5 text-[13px] text-muted">
            Day and window · results respect published occupancy
          </p>
        </div>
        <div className="grid gap-4 p-5 sm:grid-cols-4">
          ...
          <div className="flex items-end">
            <Button className="w-full" onClick={handleSearch} disabled={loading}>
              ...
              <span ref={btnLabelRef} className="t-text-swap">
                Search
              </span>
            </Button>
          </div>
      ...
      {found && results && (
        <div className="mb-4 flex items-center gap-3">
          ...
          <p className="text-[14px] text-muted">
            <span className="font-semibold text-fg">{results.length}</span> free
            classrooms
          </p>
        </div>
      )}
      ...
      {results && (
        <div className="space-y-2.5">
          {results.map((room) => (
            <div
              key={room.id}
              className="flex flex-col justify-between gap-3 rounded-2xl border border-border bg-surface px-5 py-4 sm:flex-row sm:items-center"
            >
              <div>
                <div className="flex items-center gap-2">
                  <h3 className="text-[20px] font-semibold tabular-nums tracking-tight">
                    {room.name}
                  </h3>
                  <span className="rounded-full bg-[rgba(48,209,88,0.15)] px-2 py-0.5 text-[11px] font-semibold text-ok">
                    Free
                  </span>
                </div>
      ...
      {!results && (
        <div className="py-20 text-center">
          ...
          <p className="text-[15px] text-muted">
            Pick a day and time, then search.
          </p>
        </div>
      )}
