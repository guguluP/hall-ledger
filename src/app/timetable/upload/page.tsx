...
export default function TimetableUploadPage() {
  ...
  const [status, setStatus] = useState<
    "idle" | "processing" | "ready" | "publishing" | "published" | "error"
  >("idle");
  ...
  const handlePublish = async () => {
    ...
    try {
      const res = await fetch("/api/timetable/publish", {
        ...
        body: JSON.stringify({ parse: parsePayload.parse, acceptedSuggestionIds: [] }),
      });
      const data = await res.json();
  ...
  return (
    <AppShell title="Upload timetable" subtitle="Excel workbook · all 16 first-year sections">
      <div
        {...getRootProps()}
      ...
      >
        ...
        {file ? (
          ...
          <p className="text-[15px] text-muted">Drop the 2025–26 workbook here, or click to browse</p>
        )}
        <p className="mt-1 text-[12px] text-subtle">.xlsx · .xls · .csv</p>
      </div>
      ...
      {summary && (
        <div className="grid grid-cols-3 gap-2.5 sm:max-w-md">
          <Stat label="Sections" value={String(summary.totalSections)} />
          <Stat label="Slots" value={String(summary.totalSlots)} />
          <Stat label="Conflicts" value={String(summary.totalConflicts)} ok={summary.totalConflicts === 0} />
        </div>
      )}
...
function Stat({ label, value, ok }: { label: string; value: string; ok?: boolean }) {
  return (
    <div className="rounded-[28px] border border-border bg-surface px-4 py-3 text-center">
      ...
      <p className={`mt-1 text-[22px] font-semibold tabular-nums tracking-tight ${ok ? "text-ok" : "text-fg"}`}>{value}</p>
    </div>
  );
}
